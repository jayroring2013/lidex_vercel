          ...
          <div className="flex flex-col md:flex-row items-end md:items-center gap-6 md:gap-8 pt-20 sm:pt-12 pb-10">
            ...
            <div className="flex-1 text-center md:text-left pb-4 min-w-0">
              <div className="flex flex-wrap items-center justify-center md:justify-start gap-2 mb-4">
                <span className="px-4 py-1.5 bg-primary-500/90 rounded-full text-xs font-semibold text-white">{typeText}</span>
                <span className={`px-4 py-1.5 rounded-full text-xs font-semibold text-white ${isOngoing ? 'bg-green-500/90' : 'bg-blue-500/90'}`}>
              ...
              <h1 className="text-3xl md:text-5xl font-bold text-white mb-3 leading-tight">{series.title}</h1>
              ...
              {(series.author || series.studio || series.publisher) && (
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-x-5 gap-y-1 text-sm text-gray-300 mb-5">
                  {series.author    && <span><span className="text-gray-500 mr-1.5">Author</span>{series.author}</span>}
                  {series.studio    && <span><span className="text-gray-500 mr-1.5">Studio</span>{series.studio}</span>}
                  {series.publisher && <span><span className="text-gray-500 mr-1.5">Publisher</span>{series.publisher}</span>}
                </div>
              )}
              ...
              {series.genres && series.genres.length > 0 && (
                <div className="flex flex-wrap items-center justify-center md:justify-start gap-2">
                  {series.genres.slice(0, 6).map((genre: string, i: number) => (
                    <span key={`genre-${i}`} className="px-3 py-1.5 bg-white/20 backdrop-blur-sm rounded-full text-xs font-medium text-white hover:bg-white/30 transition-colors">
                      {genre}
                    </span>
                  ))}
                </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="grid lg:grid-cols-3 gap-8 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            ...
            {/* Synopsis */}
            <div className="bg-white dark:glass rounded-2xl p-6 md:p-8 border border-gray-200 dark:border-transparent shadow-sm dark:shadow-none">
              <div className="flex items-center space-x-2 mb-4">
                <BookOpen className="w-6 h-6 text-primary-500" />
                <h2 className="text-xl font-bold text-gray-900 dark:text-white">Synopsis</h2>
              </div>
              <div className="relative">
                <div className={`text-gray-700 dark:text-gray-300 leading-relaxed text-base md:text-lg ${synopsisExpanded ? '' : 'line-clamp-4 md:line-clamp-5'}`}>
                  {formatSynopsis(series.description || series.description_vi || '')}
                </div>
                {!synopsisExpanded && (series.description || series.description_vi) && (
                  <div className="absolute bottom-0 left-0 right-0 h-12 bg-gradient-to-t from-white dark:from-dark-900 to-transparent pointer-events-none" />
                )}
              </div>
          ...
          <div className="space-y-6">
            ...
            {series.tags && series.tags.length > 0 && (
              <div className="bg-white dark:glass rounded-2xl p-6 border border-gray-200 dark:border-transparent shadow-sm dark:shadow-none">
                <div className="flex items-center space-x-2 mb-4">
                ...
                <div className="flex flex-wrap gap-2">
                  {series.tags.map((tag: string, i: number) => (
                    <span key={`tag-${i}`} className="px-3 py-1.5 bg-gray-100 dark:bg-dark-800 text-gray-700 dark:text-gray-300 rounded-lg text-sm hover:bg-gray-200 dark:hover:bg-dark-700 transition-colors cursor-pointer">
                      {tag}
                    </span>
                  ))}
                </div>
            ...
            <div className="bg-white dark:glass rounded-2xl p-6 border border-gray-200 dark:border-transparent shadow-sm dark:shadow-none">
              ...
              <div className="grid grid-cols-2 gap-3">
                <button
                  onClick={handleShare}
                  className="p-3 bg-gray-100 dark:bg-dark-800 hover:bg-gray-200 dark:hover:bg-dark-700 rounded-lg flex items-center justify-center space-x-2 transition-colors text-gray-700 dark:text-gray-300 hover:text-gray-900 dark:hover:text-white border border-gray-200 dark:border-transparent"
                >
                ...
                <button
                  ...
                  className="p-3 bg-gray-100 dark:bg-dark-800 hover:bg-gray-200 dark:hover:bg-dark-700 rounded-lg flex items-center justify-center space-x-2 transition-colors text-gray-700 dark:text-gray-300 hover:text-[#1d9bf0] border border-gray-200 dark:border-transparent"
                >
            ...
            <div className="bg-white dark:glass rounded-2xl p-6 border border-gray-200 dark:border-transparent shadow-sm dark:shadow-none">
              <div className="flex items-center space-x-2 mb-4">
                ...
                <h3 className="text-lg font-bold text-gray-900 dark:text-white">External Links</h3>
              </div>
              <div className="space-y-3">
                <a
                  href={`https://anilist.co/search/${series.item_type || 'anime'}?search=${encodeURIComponent(series.title)}`}
                  target="_blank" rel="noopener noreferrer"
                  className="flex items-center justify-between p-3 bg-gray-100 dark:bg-dark-800 hover:bg-gray-200 dark:hover:bg-dark-700 rounded-lg transition-colors group border border-gray-200 dark:border-transparent"
                >
                  <div className="flex items-center gap-2.5">
                    <span className="w-2 h-2 rounded-full bg-[#02a9ff]" />
                    <span className="text-sm text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors font-medium">AniList</span>
                  </div>
                  <ExternalLink className="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover:text-primary-500 transition-colors" />
                </a>
                <a
                ...
                >
                  <div className="flex items-center gap-2.5">
                    ...
                    <span className="text-sm text-gray-700 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-white transition-colors font-medium">MyAnimeList</span>
                  </div>
                  <ExternalLink className="w-4 h-4 text-gray-400 dark:text-gray-500 group-hover:text-primary-500 transition-colors" />
                </a>
              </div>
            ...
            {/* Last Updated */}
            <div className="bg-white dark:glass rounded-2xl p-6 border border-gray-200 dark:border-transparent shadow-sm dark:shadow-none">
              <h3 className="text-lg font-bold text-gray-900 dark:text-white mb-4">Updated</h3>
...
function InfoItem({ icon: Icon, label, value }: { icon: any; label: string; value: string }) {
  return (
    <div className="p-3 bg-gray-100 dark:bg-dark-800/50 rounded-lg border border-gray-200 dark:border-transparent">
